...
- `lib/features/tasks/screens/tasks_screen.dart:47-50` — `_loadTasks`
- `lib/features/auth/screens/api_keys_screen.dart:35,57,89` — `_loadKeys`, `_createKey`, `_revokeKey`
- `lib/features/editor/screens/node_editor_screen.dart:105-124` — `_loadPage`
- `lib/features/home/screens/dashboard_screen.dart:77,129,166` — `_loadDashboard`, `_archiveBlock`, `_deleteBlock`
- `lib/features/auth/screens/user_profile_screen.dart:64,108` — `_saveProfile`, `_changePassword`
- `lib/features/auth/screens/server_setup_screen.dart:37,75` — `_loadServers`, `_saveServer`
...
- `lib/features/search/screens/search_screen.dart:241-265` — `_search`
- `lib/presentation/screens/templates_screen.dart:42,87` — `_loadTemplates`, `_onTemplateTap`
- `lib/features/home/screens/notifications_screen.dart:36,56,72` — `_loadNotifications`, `_markRead`, `_markAllRead`
- `lib/features/auth/screens/server_management_screen.dart:33,45` — `_loadServers`, `_switchServer`
- `lib/features/library/screens/archived_screen.dart:42,62` — `_loadArchived`, `_unarchive`
- `lib/features/library/screens/trash_screen.dart:40,60,101,172` — `_loadTrash`, `_restoreNode`, `_deleteNode`, `_emptyTrash`
...
- `lib/features/settings/screens/settings_screen.dart:969` — `_AccentSwatch` uses `Colors.white` (invisible in light mode).
- `lib/features/settings/screens/settings_screen.dart:975` — `_AccentSwatch` uses `const Color(0xFF5B7D5B)` instead of the existing `noteesAccent` constant.
- `lib/features/editor/widgets/block_tree_editor.dart:402-406` — callout colors use `Colors.orange`, `Colors.green`, `Colors.blue`, `Colors.teal`.
...
- `lib/features/settings/screens/settings_screen.dart:926-927` — `_ThemeButton` is `36 × 36`.
- `lib/features/settings/screens/settings_screen.dart:1012-1013` — `_AccentSwatch` is `28 × 28`.
- `lib/features/capture/widgets/quick_capture_sheet.dart:346-348` — `_ColorButton` is `32 × 32`.
- `lib/features/editor/widgets/block_tree_editor.dart:618` — bullet/drag handle width is `28`.
- `lib/features/editor/widgets/block_tree_editor.dart:589-595` — `_BlockToolbarButton` is `36 × 36`.
...
- `lib/features/auth/screens/login_screen.dart:101-108` — password visibility toggle.
- `lib/features/auth/screens/user_profile_screen.dart:220,232,244` — password visibility toggles.
- `lib/features/home/screens/dashboard_screen.dart:287-290` — date-picker close button.
- `lib/features/search/screens/search_screen.dart:340-347` — clear-search button.
- `lib/features/search/screens/search_screen.dart:368-376` — filter button.
- `lib/features/auth/screens/api_keys_screen.dart:180-183` — delete-key button.
- `lib/features/settings/screens/settings_screen.dart:695,746,797` — close buttons in picker sheets.
- `lib/shared/widgets/node_picker.dart:133` — close button.
...
`lib/features/editor/widgets/ast_rich_text.dart:172` wraps node links in a `GestureDetector` with no `tooltip` or `Semantics` label. TalkBack/VoiceOver users won’t know the link is tappable.

**Fix:** Wrap the chip in `Semantics(button: true, label: 'Link to $label', child: ...)`.

### 7. No undo after destructive list dismissals

`lib/features/home/screens/dashboard_screen.dart:436-471` and `lib/shared/views/inbox_card_view.dart:90-150` swipe-to-delete/archive without showing an `Undo` SnackBar. The fleet component checklist requires undo after deletion.
...
- `lib/features/search/widgets/filter_bottom_sheet.dart:53` uses `Radius.circular(24)` instead of the fleet `28`.
- `lib/features/search/widgets/filter_bottom_sheet.dart:21` sets `backgroundColor: Colors.transparent` and rebuilds the surface manually; this bypasses `BottomSheetThemeData`.
...
`lib/features/editor/widgets/block_tree_editor.dart:522` uses `Material(elevation: 3, ...)` for drag feedback.
...
- `lib/shared/widgets/offline_banner.dart:19` — `AnimatedCrossFade` always animates.
- `lib/features/editor/widgets/block_tree_editor.dart:279` — `AnimatedContainer` always animates.
...
- `lib/features/home/screens/main_shell_screen.dart:124` — bottom-nav selection.
- `lib/features/editor/widgets/editor_inline_toolbar.dart:151-157` — toolbar buttons.
- `lib/shared/views/node_list_view.dart:58` / `node_card_view.dart:53` / `node_table_view.dart:149` / `node_kanban_view.dart:311` — favorite toggles.
- `lib/shared/views/node_calendar_view.dart:210-224` — month navigation.
- `lib/features/search/widgets/filter_bottom_sheet.dart:121-138,152-163,186-206` — chips, date tile, dropdown.
- `lib/shared/widgets/view_mode_sheet.dart:60` — view-mode selection.
...
- `lib/features/auth/screens/login_screen.dart:134`
- `lib/features/auth/screens/server_setup_screen.dart:191`
- `lib/features/home/screens/dashboard_screen.dart:401`
- `lib/features/auth/screens/user_profile_screen.dart:179,255`
...
`lib/features/library/screens/archived_screen.dart:153-161` builds a `Card` with `BorderRadius.circular(16)` instead of `20`.

**Fix:** Use the shared `FleetCard` widget or `BorderRadius.circular(20)`.

### 15. Empty-state widget deviates from fleet pattern

`lib/shared/widgets/empty_state.dart:27` uses a `64` px icon and `titleMedium` for the heading. The fleet empty-state pattern is a `48` px muted icon with `titleLarge`/`bodyLarge` heading and `bodyMedium` subtitle.

**Fix:** Resize icon to `48` and use `titleLarge` for the heading.

### 16. No tablet / landscape adaptation

`lib/features/home/screens/main_shell_screen.dart` always uses `NavigationBar`. Attire switches to `NavigationRail` at `600 dp` and uses responsive grid breakpoints at `600 dp` / `840 dp`.
...
| About / copyright / privacy strings | ✅ `lib/features/settings/screens/about_screen.dart:73,93-98` |
...
- `lib/shared/widgets/fleet_card.dart` implements the fleet card style correctly: radius `20`, elevation `0`, 10% outline.
